...
const NaverMap = ({hidden, setHiddenMap, mapHiddenControl, insertLocation}) => {
  ...
  const searchCoordinate = async () => {
    ...
    try{
      const response = await fetch(`https://ridinganywhere.site/Map/api/coordinate?lat=${coordinate.lat}&lng=${coordinate.lng}`);
  ...
  const onClickSearchBtn = async () => {
    ...
    if(location.trim().length>0){
      try {
        let response = await fetch(`https://ridinganywhere.site/Map/api/search?location=${location}`);
        let data = await response.json();
        console.log(data);
        if(data.items.length===0){
          console.log("⚠️ 장소로 검색된 데이터 없음");
          response = await fetch(`https://ridinganywhere.site/Map/api/address?address=${location}`)
